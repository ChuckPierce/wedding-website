...
function connectDB(done) {
  if (mongoose.connection.name !== 'site-test') {
    return done();
  }

  mongoose.connect((process.env.MONGO_URL || 'mongodb://localhost:27017/site-test'), function (err) {
...
describe('GET /api/getPosts', function () {
  ...
  beforeEach('connect and add two post entries', function (done) {
    ...
    connectDB(function () {
      var post1 = new Post({name: 'Prashant', title: 'Hello Site', content: "All cats meow 'site!'"});
      var post2 = new Post({name: 'Mayank', title: 'Hi Site', content: "All dogs bark 'site!'"});
...
describe('GET /api/getPost', function () {
  ...
  beforeEach('connect and add one Post entry', function(done){
    ...
    connectDB(function () {
      var post = new Post({ name: 'Foo', title: 'bar', slug: 'bar', cuid: 'f34gb2bh24b24b2', content: 'Hello Site says Foo' });
...
describe('POST /api/addPost', function () {
  ...
  it('Should send correctly add a post', function (done) {
    ...
    request(app)
      .post('/api/addPost')
      .send({ post: { name: 'Foo', title: 'bar', content: 'Hello Site says Foo' } })
      .set('Accept', 'application/json')
      .end(function (err, res) {
